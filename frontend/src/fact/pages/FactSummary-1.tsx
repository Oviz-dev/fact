...
const calculatePnLSums = (facts: FactDTO[], pnl: PnLDTO, months: string[]) => {
  let pnlSums = months.reduce((acc, month) => {
    acc[month] = facts
      .filter(fact => {
        // Убедимся, что проверяем не только текущую статью, но и ее дочерние
        const isCurrentPnL = fact.pnl?.id === pnl.id;
        const isSubPnL = fact.pnl?.parentId === pnl.id; // проверяем, если это подстатья
        return (isCurrentPnL || isSubPnL) && moment(fact.date).format('YYYY-MM') === month;
      })
